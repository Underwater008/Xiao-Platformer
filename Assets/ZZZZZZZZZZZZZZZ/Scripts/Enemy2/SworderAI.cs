...
public class SworderAI : MonoBehaviour
{
  ...
  public  void newSkill()
    {
        bullet = Instantiate(attackPre,newSkillPos.position,Quaternion.identity, this.gameObject.transform);

        if (face == 1)
        {
            bullet.GetComponent<AreaEffector2D>().forceMagnitude = -360;
        }
    }
    ...
    {
        //playerPos.gameObject.GetComponent<Rigidbody2D>().constraints = RigidbodyConstraints2D.FreezePositionX;
    }
